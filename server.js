...
try {
    ...
    console.warn('⚠️ @sendgrid/mail NON DISPONIBLE (optionnel)');
}
...
try {
    ...
    console.warn('⚠️ nodemailer NON DISPONIBLE (optionnel)');
}
...
// ========================================
//  ENVOI D'EMAIL (SendGrid + Nodemailer fallback)
// ========================================
...
app.post('/api/send-email', async (req, res) => {
    ...
    const htmlContent = `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: auto; background: #f9fafb; padding: 20px; border-radius: 8px;">
            ...
            <p style="color: #374151;">Un fichier <strong>${escapeHtml(fileName) || 'sans nom'}</strong> vous a été envoyé de manière sécurisée.</p>
            ...
            <p style="color: #6b7280; font-size: 13px;">⚠️ Ce lien fonctionne uniquement pendant que l'expéditeur reste connecté.</p>
        </div>
    `;
    ...
    if (nodemailer && process.env.GMAIL_USER && process.env.GMAIL_APP_PASSWORD) {
        try {
            ...
            await transporter.sendMail({
                from: `"TransferX" <${process.env.GMAIL_USER}>`,
                to: to,
...
setInterval(() => {
    ...
    for (const [roomId, room] of rooms.entries()) {
        if (now - room.createdAt > 3600000) rooms.delete(roomId);
    }
}, 300000);
...
io.on('connection', (socket) => {
    socket.on('create-room', (callback) => {
        ...
        rooms.set(roomId, { senderSocketId: socket.id, receiverSocketId: null, offer: null, answer: null, iceCandidates: [], createdAt: Date.now() });
        socket.join(roomId);
    ...
    socket.on('get-ice-candidates', ({ roomId }, callback) => {
        ...
        const candidates = room.iceCandidates.filter(c => (c.from === room.senderSocketId ? 'sender' : 'receiver') === otherRole).map(c => c.candidate);
...
server.listen(PORT, () => {
    ...
    console.log(`📧 Gmail: ${nodemailer && process.env.GMAIL_USER ? '✅ Configuré' : '❌ Non configuré'}`);
});